    ...
    <div class="bg-white/80 flex flex-col  p-10 rounded-xl">
        ...
        @foreach($orders as $order)
            ...
            <div class="flex flex-1">
                        ...
                        @foreach(json_decode($order->name) as $name)
                            @foreach($name as $name => $price)
                                <h3 class="  text-[#191919] text-md font-normal  leading-[21px]">{{ $name }} <span
                                        class="relative justify-start text-[#191919] text-md font-semibold  leading-[16.80px]"> €{{ $price }} &times; {{ $order->amount }}</span>
                                ...
                                @foreach($topping as $name => $price)
                                    ...
                                    <span
                                        class="relative justify-start text-[#7f7f7f] text-sm font-normal  leading-[21px]">{{ $name }}</span><span
                                            class="relative justify-start text-[#191919] text-sm font-semibold  leading-[16.80px]"> {{ $price }}</span>
                                    </p>
                                @endforeach
                            @endforeach
                        @endif
                ...
                <div
                    class="inline-block min-h-[3em] w-0.5 self-stretch bg-gradient-to-r from-transparent via-[#191919]/20">
                ...
                <div class="flex items-center justify-center">
                    ...
                    <span id="counter" class="text-md w-10  text-center text-gray-950 font-bold mx-4">€ {{ $price *$order->amount }}</span>
...
{{--        <div class="flex  gap-2 mb-6 items-center"><img width="120" height="100" src="https://iili.io/3FqLBsI.png" alt="">
        ...
        <div class="">
            ...
            <button
                class="w-full px-10 py-4 bg-emerald-700 rounded-2xl text-white text-base font-semibold  leading-tight">
                Daryti užsakymą
            </button>
