<div class="relative gap-3 mt-10 overflow-x-auto shadow-md sm:rounded-lg text-white/80">
    <div class="flex h-full w-full flex-1  gap-4 rounded-xl">
        <div class="auto-rows-min gap-4 w-3/4" data-tabs-toggle="#default-tab-content" role="tablist">
            <table class="w-full text-sm text-left  dark:text-gray-400 table-fixed">
                ...
                <tr>
                    ...
                    <th scope="col" class="px-0  w-[80px] py-3">
                        Žymėjimas
                    </th>
                ...
                @foreach($products as $productName)
                    <tr class="border-b border-green-500/30 bg-white/10">
                        <th scope="row"
                            class="@if($editedProductNameId === $productName->id) hidden @endif px-6 py-4 font-medium   dark:text-white">
                            {{ $productName->name }}
                        </th>
                        <td class="@if($editedProductNameId !== $productName->id) hidden @endif px-4 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                            <input type="text" wire:model.live.debounce="name" id="name"
                                   class="bg-white/80 px-4 py-1.5  pl-4 rounded-lg  sm:text-base ">
                            @error('name')
                        ...
                        <td class="@if($editedProductNameId !== $productName->id) hidden @endif px-4 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                            <input type="text" wire:model.live.debounce="price" id="price"
                                   class="bg-white/80 px-4 py-1.5  pl-4  w-[100px] rounded-lg  sm:text-base ">
                            @error('price')
                            <span class="text-sm text-red-500">hy7hy7h</span>
                            @enderror
                        </td>
                        <th scope="row"
                            class="@if($editedProductNameId === $productName->id) hidden @endif px-4 py-4 w-[50px] font-medium   dark:text-white">
                            {{ $productName->sign }}
                        </th>
                        <td class="@if($editedProductNameId !== $productName->id) hidden @endif px-0 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                            <input type="text" wire:model.live.debounce="sign" id="sign"
                                   class="bg-white/80 py-1.5  pl-4  w-[50px] rounded-lg  sm:text-base ">
                            @error('sign')
                            <span class="text-sm text-red-500">hy7hy7h</span>
                            @enderror
                        </td>
                        <th scope="row"
                            class="@if($editedProductNameId === $productName->id) hidden @endif px-16 py-4 font-medium   dark:text-white">
                            @if($productName->show)
                            ...
                            @else
                                <div class="flex items-center">
                                    <div class="h-2.5 w-2.5 rounded-full bg-gray-300 me-2"></div>
                                    Ne
                                </div>
                            @endif
                        </th>
                        ...
                        <td class="px-6 py-4">
                            @if($editedProductNameId === $productName->id)
                                <flux:button wire:click="save({{ $productName->id }})" variant="primary" class="">
                                    Išsaugoti
                                </flux:button>
                                <flux:button wire:click.prevent="cancel" variant="primary">Atšaukti</flux:button>
                            @else
                                <flux:button wire:click="editProduct({{ $productName->id }})" variant="primary">
                                    Pakeisti
                                </flux:button>
                                <flux:button wire:click="deleteProduct({{ $productName->id }})" variant="primary">
                                    Pašalinti
                                </flux:button>
                            @endif
                        </td>
        ...
        <livewire:menu.create-form.toppings-create-form/>
    </div>
</div>
